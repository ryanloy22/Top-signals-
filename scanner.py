...
Schedules  : Real-time on signal + 4-hour digest
...
def analyze_ticker(ticker: str) -> Optional[dict]:
    try:
        ...
        is_hbeta  = ticker in ["TSLA","NVDA","AMD","COIN","MSTR","PLTR","SMCI","SOXL","TQQQ"]
        ...
        macd_i = MACD(close)
        ml, ms, mh = float(macd_i.macd().iloc[-1]), float(macd_i.macd_signal().iloc[-1]), float(macd_i.macd_diff().iloc[-1])
        ...
        bull_score = sum([cross_bull*3, trend_up*2, macd_bull*2, srsi_bull*1,
                          vol_surge*1, bb_squeeze*1, (gl["signal"]=="bullish")*2, vol_spike*1])
        bear_score = sum([cross_bear*3, trend_down*2, macd_bear*2, srsi_bear*1,
        ...
        return {
            "ticker": ticker, "type": "crypto" if is_crypto else "stock",
            "signal_type": stype, "price": round(price,4),
            "direction": direction, "score": score, "rr_ratio": rr,
            "entry": round(price,4), "stop_loss": round(stop_loss,4),
            "target1": t1, "target2": t2, "target3": t3,
            "risk_per_unit": round(risk,4), "atr": round(atr_val,4),
            "vol_ratio": vol_ratio, "candle_pct": candle_pct,
            "vol_spike": vol_spike, "bb_squeeze": bb_squeeze,
            "ema_cross": cross_bull or cross_bear,
            "macd_signal": "bull" if macd_bull else "bear",
            "stochrsi_k": round(sk,3), "green_lights": gl,
            "timeframe": tf, "scanned_at": datetime.datetime.utcnow().isoformat()+"Z",
        }
...
def format_digest(results, sentiment):
    lines = [
        "="*45,
        "TRADESIGNAL 4-HOUR DIGEST",
...
def send_sms(body):
    ...
    if not all([c["TWILIO_SID"],c["TWILIO_AUTH"],c["TWILIO_FROM"],c["TWILIO_TO"]]):
        print("  ⚠ Twilio credentials missing in .env"); return
    try:
...
def send_email(subject, body):
    ...
    if not all([c["SENDGRID_API_KEY"],c["ALERT_EMAIL_FROM"],c["ALERT_EMAIL_TO"]]):
        print("  ⚠ SendGrid credentials missing in .env"); return
    try:
...
def send_alert(r, sentiment):
    ...
    subj = f"ALERT: {r['signal_type']} | {r['ticker']} {r['direction']} R/R:{r['rr_ratio']}"
    print(f"\n  Sending alert: {r['ticker']}...")
    send_sms(body[:1600])
    send_email(subj, body)

def send_digest(results, sentiment):
    body = format_digest(results, sentiment)
    subj = f"TradeSignal Digest | {len(results)} signals | {sentiment['icon']} {sentiment['bias']}"
    print(f"\n  Sending digest ({len(results)} signals)...")
    send_sms(body[:1600])
    send_email(subj, body)
...
def run_scanner(send_alerts=True):
    print("\n" + "━"*55)
    print("  TradeSignal Scanner v3.0")
    ...
    for i, ticker in enumerate(ALL_TICKERS, 1):
        ...
        if r:
            ...
            print(f"{flag} {r['direction']:<5} Score:{r['score']} R/R:{r['rr_ratio']} GL:{gl_icon} Vol:{r['vol_ratio']}x")
    ...
    print(f"\n{'━'*55}  TOP {len(top)} SETUPS  {'━'*55}")
    for r in top:
        gl = r["green_lights"]
        print(f"\n  {r['ticker']} ({r['type']}) | {r['direction']} | {r['signal_type']}")
        print(f"  Score:{r['score']}  R/R:{r['rr_ratio']}:1  TF:{r['timeframe']}")
        print(f"  Entry:{r['entry']}  Stop:{r['stop_loss']}  T1:{r['target1']}  T2:{r['target2']}  T3:{r['target3']}")
    ...
    print(f"\n✅ {len(results)} signals saved | 🚨 Spikes:{len(vol_spikes)}  ⚡ Crosses:{len(ema_crosses)}")
    print("━"*55 + "\n")
    return output
